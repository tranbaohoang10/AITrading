...
## 04/09/2026 — Latest-candle and axis interaction correction

- Corrected wheel and keyboard zoom to clamp the visible window to the imported dataset and preserve the newest candle at the right edge while the chart follows the latest region; historical navigation exits that follow mode and retains a bounded cursor-centered anchor.
- Corrected pointer interaction so the main chart body pans time horizontally only. The right price-axis strip independently establishes manual display-price scaling, with an in-canvas Auto action and Reset restoring automatic visible high/low fitting.
- Focused chart regression: 16/16 tests PASS. Browser QA replayed the current LuxAlgo reference and local `DEMO_USD` chart at 1536×730; LuxAlgo showed latest-right-edge behavior and separate price-axis Auto control, while Quant showed continuous candles, horizontal body navigation, and separate price-axis manual scaling. Local CUA wheel delivery remained inconsistent after keyboard-path verification, so the existing real-wheel 100→84 evidence and the new regression test remain the direct wheel evidence.
