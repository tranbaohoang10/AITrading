"""Actual MQL5 artifact HTTP/PostgreSQL/restart smoke, only on owned local test DB.

Does not execute MQL5 or submit source to an external target. No secret is written to output.
"""
import argparse
import hashlib
...
def main():
    ...
    call('GET','/health')
    password=secrets.token_urlsafe(32);email=f'mql5-smoke-{uuid.uuid4().hex}@example.test'
    call('POST','/auth/register',{'email':email,'displayName':'Synthetic MQL5 restart','password':password},202)
    call('POST','/auth/login',{'email':email,'password':password},204,True);del password
    account=call('GET','/auth/me')['id']
    source=call('POST','/strategies',{'requestId':str(uuid.uuid4()),'title':'Synthetic MQL5 restart'})
    strategy='/strategies/'+source['strategyId']
    source=call('POST',strategy+'/versions',{'requestId':str(uuid.uuid4()),'expectedRevision':1,'title':'Synthetic MQL5 restart',
        'draftText':(ROOT/'backend/src/test/resources/dsl/price-action.json').read_text(encoding='utf-8'),'mode':'VALIDATED'})
    path=strategy+'/versions/2/mql5'
    ...
    assert 'RESEARCH ONLY: CSV simulation, not native Strategy Tester or live trading.' in artifact['code']
    assert 'mql5-research-1.0.0' in artifact['code'] and 'OrderSend(' not in artifact['code']
    ...
    report.write_text(json.dumps({'passed':True,'synthetic':True,'realHttpPostgres':True,'actualApiDownUpObserved':saw_down,
        ...
        'officialMQL5Runtime':'NOT_RUN'},indent=2)+'\n',encoding='utf-8')
    print('PASS: actual HTTP/PG MQL5 artifact hash, replay, API restart, session persistence and deletion; synthetic account signed out. MQL5 runtime NOT RUN.')
