...
class AiTradingApplicationTests {
    ...
    void databaseMigrationAndRepeatValidationAreRealAndIdempotent() {
        ...
        assertThat(jdbc.queryForObject("SELECT to_regclass('trading.provider_market_candle') IS NOT NULL",Boolean.class)).isTrue();
        assertThat(jdbc.queryForObject("SELECT to_regclass('trading.provider_market_sync_state') IS NOT NULL",Boolean.class)).isTrue();
        flyway.validate();
