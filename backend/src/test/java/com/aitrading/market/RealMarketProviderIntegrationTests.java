...
class RealMarketProviderIntegrationTests {
    ...
    @Autowired MarketHistoricalSyncService sync;@Autowired MarketProviderCandleStore store;@Autowired MarketSymbolRegistry registry;@Autowired AlpacaHistoryProvider alpaca;@Autowired BinanceArchiveProvider binance;@Autowired CapitalMarketDataClient capital;@Autowired CtraderMarketDataClient ctrader;@Autowired DukascopyHistoryProvider dukascopy;
    ...
    private Instant available(MarketSymbolRegistry.Route route){return switch(route.provider()){case "ALPACA"->alpaca.historicalAvailableFrom(route.providerSymbol());case "BINANCE"->binance.historicalAvailableFrom(route.providerSymbol());case "CAPITAL"->capital.historicalAvailableFrom(route.providerSymbol());case "CTRADER"->BASELINE;default->dukascopy.historicalAvailableFrom(route.providerSymbol());};}
