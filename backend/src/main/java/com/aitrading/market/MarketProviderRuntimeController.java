...
public final class MarketProviderRuntimeController {
    ...
    @GetMapping("/history") public List<MarketDataProvider.Candle> history(@RequestParam String symbol,@RequestParam String timeframe,@RequestParam Instant from,@RequestParam Instant to,@RequestParam(defaultValue="1000")int limit){range(from,to,366);var route=registry.resolve(symbol);var rows=store.read(route,timeframe,from,to,limit);if(rows.isEmpty()){var result=sync.sync(symbol,from,to);if(!result.status().equals("READY"))return List.of();rows=store.read(route,timeframe,from,to,limit);}return rows;}
    ...
    @GetMapping("/live-state") public Map<String,Object> liveState(@RequestParam String symbol){var route=registry.resolve(symbol);var keys=live.keys(route.provider(),route.providerSymbol());return Map.of("provider",route.provider(),"symbol",route.canonicalSymbol(),"redisHealth",live.health(),"keys",keys,"latest",Objects.toString(live.read(keys.latest()),""),"current",Objects.toString(live.read(keys.current()),""),"status",Objects.toString(live.read(keys.status()),""));}
