...
public final class CapitalMarketDataClient implements MarketDataProvider,HistoricalAvailabilityProvider,HistoricalSourceTimeframeProvider {
    ...
    private Instrument instrument(Spec spec){var increment=BigDecimal.ONE.movePointLeft("JPY".equals(spec.quote())?3:5);return new Instrument("CAPITAL:"+spec.epic(),spec.display(),spec.epic(),"CAPITAL",spec.asset(),spec.base(),spec.quote(),"Capital.com",spec.quote(),"CFD","UTC",increment,null,null,null,null,"PROVIDER_CONTRACT",null,null,null,"QUANTITY_ONLY",List.of("HISTORICAL","REALTIME"),TIMEFRAMES,"VERIFIED_FROM_2024_01_03",spec.name());}
    ...
    private static Map<String,Spec> specs(){var rows=List.of(new Spec("EUR/USD","EUR/USD","EURUSD","FOREX","EUR","USD","Euro / US Dollar CFD"),new Spec("GBP/USD","GBP/USD","GBPUSD","FOREX","GBP","USD","British Pound / US Dollar CFD"),new Spec("USD/JPY","USD/JPY","USDJPY","FOREX","USD","JPY","US Dollar / Japanese Yen CFD"),new Spec("AUD/USD","AUD/USD","AUDUSD","FOREX","AUD","USD","Australian Dollar / US Dollar CFD"),new Spec("USD/CAD","USD/CAD","USDCAD","FOREX","USD","CAD","US Dollar / Canadian Dollar CFD"),new Spec("USD/CHF","USD/CHF","USDCHF","FOREX","USD","CHF","US Dollar / Swiss Franc CFD"),new Spec("NZD/USD","NZD/USD","NZDUSD","FOREX","NZD","USD","New Zealand Dollar / US Dollar CFD"),new Spec("XAU/USD","XAU/USD","GOLD","COMMODITY","XAU","USD","Gold CFD"),new Spec("XAG/USD","XAG/USD","SILVER","COMMODITY","XAG","USD","Silver CFD"),new Spec("XPT/USD","XPT/USD","PLATINUM","COMMODITY","XPT","USD","Platinum CFD"),new Spec("XPD/USD","XPD/USD","PALLADIUM","COMMODITY","XPD","USD","Palladium CFD"));var result=new LinkedHashMap<String,Spec>();for(var row:rows)for(var alias:List.of(row.canonical(),row.display(),row.epic(),"CAPITAL:"+row.epic(),row.name()))result.put(normalize(alias),row);return Map.copyOf(result);}
}
