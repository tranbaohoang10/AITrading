# Master Product Backlog â€” Autonomous Prototype

Created 30/08/2026, before product code changes. Owner: Product Owner; execution: Codex.
Source: full AUTONOMOUS PRODUCT BUILD request (sections Aâ€“Q), current Constitution,
...
| PB-008 | Provider-neutral Quant / AI assistant: Gemini, optional OpenAI | PB-004, PB-005 | P1 | IN_PROGRESS | [#12](https://github.com/tranbaohoang10/AITrading/issues/12) | Default3.5Flash configurable;211backend/209frontend/44Python PASS, Java118/npm0. Real Gemini two-turn synthetic structured/context/isolation/replay/JVMrestart smoke PASS. Publication/exactSHA/CI pending | Server-only keys, owner/account/session/CSRF, bounded context/output/rates/timeouts, no tools/URL override, synthetic-only free-tier smoke | Both provider contracts, HTTP+PG/races/migration/frontend/security PASS; real smoke evidence gemini35-real-smoke.json; close only after publication/CI |
| PB-009 | Natural-language strategy generation | PB-007, PB-008 | P1 | PLANNED | not created | Prompt â†’ structured DSL draft â†’ validation â†’ explicit user acceptance/version; explain missing/ambiguous rules | AI output never executable code; ownership, injection and resource controls | Multiple strategy families and hybrid prompts; invalid provider DSL, acceptance/rejection, no auto-backtest |
...
| PB-011 | Owned backtest jobs and API integration | PB-003, PB-007, PB-010 | P1 | DONE | [#13](https://github.com/tranbaohoang10/AITrading/issues/13) | Delivered bfebd5f/bcdeaff; CI33365234612/33365494850 PASS; Issue completed;122 backend/40 Python; snapshot jobs and actual restart | Ownership, subprocess fixed args, no shell injection, resource budgets/races | Actual Javaâ†”Pythonâ†”DB integration; cancellation/timeouts/duplicate requests/cross-user |
...
| PB-025 | System integration and failure recovery | PB-004, PB-009, PB-012, PB-014, PB-017, PB-018, PB-019, PB-022, PB-023, PB-024 | P1 | PLANNED | not created | End-to-end accountâ†’chatâ†’DSLâ†’backtestâ†’journalâ†’export/RAG; restarts/errors recover without mixing users | Trust boundaries and data-loss safeguards across real services | Browser+API+DB+Python journeys, provider unavailable, migrations, concurrency |
...
Initial order: PB-001 â†’ PB-002 â†’ PB-003 â†’ PB-004 â†’ PB-005 â†’ PB-006 â†’ PB-007
â†’ PB-010 â†’ PB-008 â†’ PB-009 â†’ PB-011 â†’ PB-012 â†’ PB-013 â†’ PB-014
â†’ PB-015 â†’ PB-016 â†’ PB-017 â†’ PB-018 â†’ PB-023 (as soon as dependencies finish)
â†’ PB-022 â†’ PB-024 â†’ PB-019 â†’ PB-025 â†’ PB-026.
...
## 31/08/2026 â€” Current delivery and blocker checkpoint
