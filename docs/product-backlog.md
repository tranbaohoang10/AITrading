...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PB-001 | Reuse and verify frontend foundation / trading shell | — | P0 | IN_PROGRESS | [#4](https://github.com/tranbaohoang10/AITrading/issues/4) | Recover valid mvp-ui source; reproducible install/build/lint/tests; modern neutral styling, keyboard/responsive browser checks; demo actions visibly labelled; replaceable brand | Text injection, clipboard errors, dependency audit, no external calls or secret storage | Preserve 10 tests; add boundary/negative tests; real browser desktop/tablet/mobile |
| PB-002 | Backend and database foundation | — | P0 | READY | not created | Java21 Spring Boot/Gradle Wrapper; PostgreSQL/Flyway; isolated test DB; health and structured errors; reproducible local startup | Deny private endpoints by default, bounded inputs, no debug/data leaks, configuration secrets | Wrapper tests/build, DB migrations/restart, health/error/security integration |
| PB-003 | Authentication and user management | PB-001, PB-002 | P0 | PLANNED | not created | Register/login/logout/current user; Argon2id; backend sessions/ownership principal; account settings and password change | CSRF, fixation/revocation, brute force/enumeration, hash secrecy, concurrent uniqueness | Auth API and browser journeys, two users, expiry/forgery/rate-limit/concurrency |
| PB-004 | Persistent conversations and messages | PB-003 | P0 | PLANNED | not created | New/list/reopen/continue/rename/delete; timestamps/title/last message; bounded pagination; distinct contexts after restart | Owner predicates on every operation, XSS, mass assignment, idempotent writes | Two-user CRUD/context, restart, duplicates, invalid IDs, pagination/concurrency |
| PB-005 | Versioned neutral Strategy DSL | PB-002 | P0 | PLANNED | not created | Typed bounded measurable conditions/risk/execution; schema+semantic validation; explicit unsupported cases; immutable versions | No eval/scripts; bounded trees/periods/symbols, canonical hashing | Known valid/invalid families, malformed/deep DSL, deterministic hashes |
| PB-006 | Market data import and chart datasets | PB-003 | P0 | PLANNED | not created | Owned OHLCV CSV import, provenance/hash, timeframe/UTC boundaries, real candlestick navigation; honest synthetic seed | File size/type/path, isolation, invalid values, duplicate candles, no arbitrary URL fetch | Import errors, gaps/order/precision, pagination/rendering, other-user denial |
| PB-007 | Strategy storage, editing and My Script workspace | PB-001, PB-003, PB-005 | P0 | PLANNED | not created | Create/edit/validate/save versions; explicit draft vs executable; chart alongside relevant strategy; conflicts do not overwrite | Ownership, version concurrency, code-view escaping | CRUD, stale version, invalid edits, reload, mobile workspace |
| PB-008 | Quant / AI assistant provider boundary | PB-004, PB-005 | P1 | PLANNED | not created | Configurable maintained AI integration; bounded correct per-conversation context; structured validated response; real response/error distinction | Server-side keys, provider timeout/retry, prompt/data isolation, bounded quotas, no arbitrary tools | Contract mocks plus real configured-provider smoke; context isolation, unavailable provider, retry/cancellation |
...
| PB-010 | Deterministic Python backtest engine | PB-005, PB-006 | P0 | PLANNED | not created | Reproducible bar events, next-bar execution, indicator warm-up, costs/sizing/leverage/SL/TP ambiguity, hashes/run card | Bounds/timeouts, no look-ahead/eval, fail unsupported multi-symbol/timeframe combos explicitly | Hand-computed trades, prefix invariance, missing/duplicate candles, same-candle exits, long/short and costs |
| PB-011 | Owned backtest jobs and API integration | PB-003, PB-007, PB-010 | P1 | PLANNED | not created | Version+dataset snapshot execution; state/progress/failure; bounded worker; restart-safe persisted results; cancel/retry/idempotency | Ownership, subprocess fixed args, no shell injection, resource budgets/races | Actual Java↔Python↔DB integration; cancellation/timeouts/duplicate requests/cross-user |
