...
export const inputClass = 'mt-1.5 min-h-10 w-full rounded-md border border-slate-700 bg-slate-950 px-3 text-slate-100 outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400 disabled:opacity-60'
export const buttonClass = 'min-h-10 rounded-md border border-slate-700 bg-slate-800 px-4 text-sm font-semibold text-slate-100 transition hover:border-slate-600 hover:bg-slate-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400 disabled:opacity-50'
...
export function AuthForm({ onSignedIn }: { onSignedIn: () => Promise<void> }) {
  ...
  return <main className="terminal-grid grid min-h-screen place-items-center bg-slate-950 px-5 py-10 text-slate-100">
    <section className="terminal-panel w-full max-w-md p-6 sm:p-8" aria-label="Account access">
      <div className="mb-8 flex items-center gap-3"><span className="quant-mark">{brand.initials}</span><span className="text-base font-semibold tracking-tight">{brand.name}</span></div>
      <p className="eyebrow">Private research</p>
      <h1 className="mt-2 text-2xl font-semibold tracking-tight">{register ? 'Create an account' : 'Sign in'}</h1>
      <p className="mt-2 text-sm text-slate-400">Sign in to your trading workspace.</p>
      <form onSubmit={submit} className="mt-8 space-y-5" aria-busy={busy}>
        <fieldset disabled={busy} className="space-y-5">
          ...
          <button className={`${buttonClass} primary-button w-full`} type="submit">{busy ? 'Please wait…' : register ? 'Create account' : 'Sign in'}</button>
        </fieldset>
      ...
      <details className="help-details mt-8"><summary>Security & risk</summary><p>Research tool only. Historical results do not guarantee future returns. Do not reuse a password from another service.</p></details>
    </section>
  </main>
}
