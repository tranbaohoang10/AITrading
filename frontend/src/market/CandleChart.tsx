...
export function CandleChart({ page, instrument, markers = [], frozen = false, dataSource = 'imported', sourceLabel, timeframe = '1h', settings = defaultChartSettings, indicators = [], activeTool = 'cursor', drawings = [], selectedDrawingId, magnet = 'off', stayInMode = false, onAddDrawing, onUpdateDrawing, onCommitDrawingEdit, onSelectDrawing, onDeleteSelected, onDeleteDrawing, onUndo, onRedo, onCancelTool, onCaptureRequest, onToggleIndicator, onRemoveIndicator, onOpenIndicators, onOpenSettings, onRequestOlder, fitDrawingPrices = false, lockedDrawingHandles = [], positionLabel, replaySelecting = false, replayCutTime, onSelectReplayTime }: {
  instrument?: import('./liveMarket').Instrument
 ...
 activeTool?: DrawingTool; drawings?: Drawing[]; selectedDrawingId?: string | null; magnet?: MagnetMode; stayInMode?: boolean
  ...
  replaySelecting?: boolean; replayCutTime?: string; onSelectReplayTime?: (time: string) => void
}) {
  const total = page.items.length
  const displaySymbol = instrument?.displaySymbol ?? page.dataset.symbol.replace(/-/g, '/')
  ...
  const timeTicks = useMemo(() => {
    ...
    for (let i = 0; i <= count; i++) {
      ...
      if (item) {
        ticks.push({
          time: item.time,
          label: formatChartDate(item.time, settings.timezone === 'EXCHANGE' ? settings.exchangeTimezone ?? 'UTC' : settings.timezone, {
  ...
  }, [visibleItems, settings.timezone, settings.exchangeTimezone, width])
  ...
  const formatTime = (value: string) => formatChartDate(value, settings.timezone === 'EXCHANGE' ? settings.exchangeTimezone ?? 'UTC' : settings.timezone, { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false })
  ...
  const pointerDown = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    if (replaySelecting) {
      if (px >= left && px <= left + width) {
        const selected = clamp(Math.round(visibleStart + (screen.x - left) / width * visibleCount - .5), 0, total - 1)
        onSelectReplayTime?.(page.items[selected].time)
      }
  ...
  return <div className="flex h-full min-h-0 w-full flex-1 flex-col">
    <div className="relative flex min-h-0 flex-1 flex-col select-none">
      ...
      <svg ref={svg} onWheel={handleWheel} viewBox={`0 0 ${viewWidth} ${totalHeight}`} preserveAspectRatio="none" style={{ background: settings.background, touchAction: 'none' }} className={`h-full min-h-0 w-full flex-1 border border-slate-800 outline-none focus-visible:border-slate-600 ${replaySelecting ? 'cursor-col-resize' : priceAxisHover || pan.current?.mode === 'scale' ? 'cursor-ns-resize' : navigationTool(activeTool) ? (pan.current ? 'cursor-grabbing' : 'cursor-crosshair') : activeTool === 'aiCapture' ? 'cursor-crosshair' : 'cursor-cell'}`} role="img" tabIndex={0} aria-label={`${displaySymbol} ${frozen ? 'frozen backtest' : dataSource} ${settings.chartType === 'candles' ? 'candlesticks' : settings.chartType}, ${Math.ceil(visibleCount)} candles in ${settings.timezone} (${renderEnd - renderStart} of ${total} loaded). Smooth cursor-centered wheel zoom and horizontal time pan; right price axis controls display scale.`}
        ...
        {futureX > left + 8 && futureX < left + width - 8 && <g data-testid="future-space"><line x1={futureX} x2={futureX} y1={top} y2={chartBottom} stroke={settings.separatorColor} strokeDasharray="5 5" opacity=".65"/><text x={futureX + 6} y={top + 16} fill="#77736d" fontSize="9">Future</text></g>}
        ...
        {replayCutTime && xTime(replayCutTime) < left + width && <g data-testid="replay-cut-overlay" pointerEvents="none"><rect x={Math.max(left, xTime(replayCutTime))} y={top} width={left + width - Math.max(left, xTime(replayCutTime))} height={chartBottom - top} fill="#101827" opacity=".72"/>{xTime(replayCutTime) >= left && <><line x1={xTime(replayCutTime)} x2={xTime(replayCutTime)} y1={top} y2={chartBottom} stroke="#60a5fa" strokeWidth="1.5"/><rect x={Math.min(xTime(replayCutTime) + 5, left + width - 76)} y={top + 5} width="72" height="18" rx="3" fill="#1d4ed8"/><text x={Math.min(xTime(replayCutTime) + 41, left + width - 40)} y={top + 18} textAnchor="middle" fill="#eff6ff" fontSize="9">Replay start</text></>}</g>}
        ...
        {(settings.showPriceLine || settings.showLastValue) && <g>{settings.showPriceLine && <line x1={left} x2={left + width} y1={yPrice(lastClose)} y2={yPrice(lastClose)} stroke="#9a958c" strokeDasharray="3 3" opacity=".7"/>}{settings.showLastValue && <LivePriceAxisBadge price={formatPrice(lastClose)} x={left + width + 3} y={clamp(yPrice(lastClose) - 8, top, chartBottom - 32)} width={right - 8} color={lastColor} timezone={settings.timezone === 'EXCHANGE' ? settings.exchangeTimezone ?? 'UTC' : settings.timezone} />}</g>}
        ...
        {crosshair && settings.showCrosshair && navigationTool(activeTool) && !pan.current && <g pointerEvents="none"><line x1={crosshair.x} x2={crosshair.x} y1={top} y2={paneBottom} stroke="#99958d" strokeDasharray="3 3" opacity=".58"/><line x1={left} x2={left + width} y1={crosshair.y} y2={crosshair.y} stroke="#99958d" strokeDasharray="3 3" opacity=".58"/><g data-testid="crosshair-price"><rect x={left + width + 3} y={clamp(crosshair.y - 8, top, axisBottom - 16)} width={right - 8} height="16" rx="2" fill="#323337"/><text x={left + width + 7} y={clamp(crosshair.y - 8, top, axisBottom - 16) + 12} fill="#e1ddd5" fontSize="10" fontFamily="monospace">{formatPrice(crossPrice!)}</text></g><g data-testid="crosshair-time"><rect x={clamp(crosshair.x - 64, 1, viewWidth - 129)} y={axisBottom + 4} width="128" height="16" rx="2" fill="#323337"/><text x={clamp(crosshair.x - 64, 1, viewWidth - 129) + 64} y={axisBottom + 15} textAnchor="middle" fill="#e1ddd5" fontSize="9">{formatTime(timeAtPosition(visibleStart + (crosshair.x - left) / width * visibleCount - .5))}</text></g></g>}
      </svg>
      {contextMenu && <div role="menu" aria-label={`${contextMenu.kind} chart context menu`} data-testid="chart-context-menu" className="absolute z-50 min-w-44 rounded-lg border border-slate-700 bg-slate-900 p-1 shadow-2xl" style={{ left: `${Math.min(Math.max(contextMenu.x, 4), Math.max(4, viewWidth - 190)) / Math.max(viewWidth, 1) * 100}%`, top: `${Math.min(Math.max(contextMenu.y, 4), Math.max(4, totalHeight - 160)) / Math.max(totalHeight, 1) * 100}%` }} onPointerDown={event => event.stopPropagation()}>
        ...
        {(contextMenu.kind === 'chart' || contextMenu.kind === 'time') && <button type="button" role="menuitem" onClick={() => { resetView(); setContextMenu(null) }} className="flex min-h-8 w-full items-center rounded-md px-2 text-left text-xs text-slate-300 hover:bg-slate-800">Go to latest</button>}
