...
export function DatasetChart() {
  ...
  return <section aria-label="Chart" data-testid="chart-view" className="flex h-full min-h-0 flex-col overflow-y-auto">
    <div className="flex flex-wrap items-center gap-2 border-b border-slate-800 bg-slate-900 px-3 py-2">
      <label className="flex min-h-9 min-w-0 flex-1 basis-full items-center gap-2 text-[11px] font-semibold text-slate-500 sm:basis-56">Dataset<select aria-label="Dataset" className="min-h-9 min-w-0 flex-1 rounded-md border border-slate-700 bg-slate-950 px-2 text-xs text-slate-100" value={selected?.id ?? ''} disabled={blocked} onChange={event => { const item = market.items.find(value => value.id === event.target.value); if (item) market.select(item) }}>
      ...
      {selected && <><span className="status-chip">{selected.symbol}</span><span className="status-chip">{selected.timeframe}</span></>}
      <button className={`${buttonClass} primary-button`} disabled={blocked} onClick={() => setImporting(true)}>Import CSV</button>
      <button aria-label="Refresh datasets" title="Refresh datasets" className={buttonClass} disabled={market.listLoading || blocked} onClick={() => void market.loadList()}>Refresh</button>
    ...
    <div className="flex flex-1 flex-col gap-3 p-3 sm:p-4">
      ...
      {!selected && !market.listLoading && <div className="my-auto py-10 text-center"><span className="sr-only">Your market datasets</span><div className="mx-auto mb-4 grid h-10 w-10 place-items-center rounded-full border border-slate-700 text-slate-500">⌁</div><h2 className="text-base font-semibold">No market data</h2><p className="mt-1 text-sm text-slate-500">{market.items.length ? 'Select a dataset to begin.' : 'Import an OHLCV dataset to begin.'}</p><button className={`${buttonClass} primary-button mt-4`} disabled={blocked} onClick={() => setImporting(true)}>Import CSV</button></div>}
      {selected && <>
