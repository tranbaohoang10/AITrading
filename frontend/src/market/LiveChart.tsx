...
function RealtimeStatus({ status, firstEventAt, lastUpdate, activeCandleTime, partial, timeframe, timezone }: { timezone: string; status: LiveConnectionStatus; firstEventAt?: number; lastUpdate?: number; activeCandleTime?: number; partial?: boolean; timeframe: Timeframe }) {
  ...
  return <div data-testid="realtime-status" role="status" aria-label={`Realtime · ${label}`} className="grid shrink-0 grid-cols-[auto_1fr] items-center gap-x-2 rounded-md border border-slate-800 bg-slate-950/60 px-2 py-1 text-[9px] leading-tight text-slate-400"><span className="flex items-center gap-1.5 uppercase tracking-[0.12em] text-slate-500"><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${liveClass[effectiveStatus]}`}/>Realtime</span><span className="font-semibold text-slate-200">{label}</span><span className="col-span-2 mt-0.5 whitespace-nowrap text-slate-500">First<span className="hidden sm:inline"> tick</span> {time(firstEventAt ?? lastUpdate)} · Last<span className="hidden sm:inline"> tick</span> {time(lastUpdate)} · Candle {time(activeCandleTime)}{partial ? ' (partial)' : ''}</span></div>
}
