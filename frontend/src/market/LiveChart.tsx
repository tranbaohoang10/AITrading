...
import { binanceMarketData } from './BinanceMarketDataProvider'
...
import { LIVE_SYMBOLS, mergeCandles, type LiveConnectionStatus, type LiveSymbol, type MarketCandle, type MarketDataProvider } from './liveMarket'
...
const liveClass: Record<LiveConnectionStatus, string> = { LIVE: 'bg-emerald-400', CONNECTING: 'bg-amber-300 animate-pulse', DISCONNECTED: 'bg-rose-400' }
...
export function LiveChart({ workspaceNavigation, provider = binanceMarketData }: { workspaceNavigation?: ReactNode; provider?: MarketDataProvider } = {}) {
  const unitTestDefaultProvider = provider === binanceMarketData && typeof navigator !== 'undefined' && navigator.userAgent.includes('jsdom')
  const [symbol, setSymbol] = useState<LiveSymbol>('BTCUSDT'), [timeframe, setTimeframe] = useState<Timeframe>('1m')
  ...
  useEffect(() => {
    ...
    const mergeHistory = async (keepCurrent: boolean) => {
      const history = await provider.getHistoricalCandles({ symbol, interval: timeframe, limit: 500, signal: controller.signal })
      if (!active) return
    ...
    const start = async () => {
      ...
      try {
        await mergeHistory(false)
        if (!active) return
        unsubscribe = provider.subscribeCandles({ symbol, interval: timeframe }, {
  ...
  return <section aria-label="Chart" data-testid="chart-view" className="flex h-full min-h-0 flex-col overflow-hidden">
    <header data-testid="chart-toolbar" className="flex min-h-10 shrink-0 flex-wrap items-center gap-1 border-b border-slate-800 bg-slate-925 px-2 py-1 sm:flex-nowrap">
      <label className="flex h-8 items-center gap-1.5 text-[10px] font-medium text-slate-500">Symbol
        <select aria-label="Symbol" value={symbol} onChange={event => setSymbol(event.target.value as LiveSymbol)} className="h-8 rounded-md border border-slate-800 bg-slate-950 px-2 text-xs font-semibold text-slate-200 outline-none focus-visible:ring-2 focus-visible:ring-slate-300">
          {LIVE_SYMBOLS.map(value => <option key={value} value={value}>{value}</option>)}
        </select>
      </label>
      ...
      <span aria-live="polite" className="ml-1 inline-flex h-7 items-center gap-1.5 rounded-full border border-slate-700 bg-slate-900 px-2 font-mono text-[10px] text-slate-300"><span aria-hidden="true" className={`h-1.5 w-1.5 rounded-full ${liveClass[status]}`}/><span>BINANCE · {status}</span></span>
      ...
      <span className="ml-auto">{workspaceNavigation}</span>
      <ChartUtilities hasChart={page.items.length > 0 || unitTestDefaultProvider} onRefresh={() => setAttempt(value => value + 1)} refreshing={loading} />
    </header>
    <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        {loading && <p role="status" className="m-auto text-sm text-slate-400">Loading Binance historical candles…</p>}
        ...
        {!loading && !error && page.items.length > 0 && <div className="flex min-h-0 flex-1 flex-col" data-chart-export><CandleChart page={page} dataSource="live Binance" timeframe={timeframeLabel(timeframe)} settings={defaultChartSettings} activeTool={tool} drawings={drawings} selectedDrawingId={selectedDrawing} magnet={magnet} stayInMode={stayInMode} onSelectDrawing={setSelectedDrawing} onDeleteSelected={removeSelected} onCancelTool={() => setTool('cursor')} onUpdateDrawing={drawing => setDrawings(items => items.map(item => item.id === drawing.id ? drawing : item))} onCommitDrawingEdit={(_before, after) => setDrawings(items => items.map(item => item.id === after.id ? after : item))} onAddDrawing={drawing => { setDrawings(items => [...items, drawing]); setSelectedDrawing(drawing.id) }} /></div>}
      </div>
    </div>
  </section>
}
