...
import { COINBASE_DEFAULT_SYMBOLS, DEFAULT_INSTRUMENTS, displayMarketSymbol, formatMarketPrice, mergeCandles, type Instrument, type LiveConnectionStatus, type LiveSymbol, type MarketCandle, type MarketDataProvider } from './liveMarket'
...
export function LiveChart({ workspaceNavigation, provider = marketDataProvider }: { workspaceNavigation?: ReactNode; provider?: MarketDataProvider } = {}) {
  ...
  const [cells, setCells] = useState<Record<string, ChartCellState>>({ c1: createChartCell() }), [symbols, setSymbols] = useState<LiveSymbol[]>([...COINBASE_DEFAULT_SYMBOLS]), [instruments, setInstruments] = useState<Instrument[]>(DEFAULT_INSTRUMENTS), [instrumentCatalog, setInstrumentCatalog] = useState<Instrument[]>(DEFAULT_INSTRUMENTS)
  ...
  const selectInstrument = (instrument: Instrument) => { setInstrumentCatalog(current => current.some(item => item.symbol === instrument.symbol) ? current : [...current, instrument]); setInstruments(current => current.some(item => item.symbol === instrument.symbol) ? current : [...current, instrument]); setSymbols(current => current.includes(instrument.symbol) ? current : [...current, instrument.symbol]); setSymbol(instrument.symbol); setSymbolSearchOpen(false); setSymbolSearch('') }
  ...
  return <section aria-label="Chart" data-testid="chart-view" className="relative flex h-full min-h-0 flex-col overflow-hidden">
    <header data-testid="chart-toolbar" className="flex min-h-10 shrink-0 items-center gap-1 border-b border-slate-800 bg-slate-925 px-2 py-1">
      <div data-testid="chart-main-controls" className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto chart-tools">
        <button type="button" aria-label="Open Symbol Search" title="Symbol Search" onClick={() => setSymbolSearchOpen(true)} className={`${iconButton} shrink-0`}><Icon name="search" className="h-4 w-4" /></button>
        <label className="flex h-8 min-w-0 items-center gap-1.5"><span className="sr-only">Symbol</span><select aria-label="Symbol" value={symbol} onChange={event => setSymbol(event.target.value as LiveSymbol)} className="h-8 max-w-36 rounded-md border border-slate-800 bg-slate-950 px-2 text-xs font-semibold text-slate-200 outline-none focus-visible:ring-2 focus-visible:ring-slate-300">{symbols.map(value => <option key={value} value={value}>{displayMarketSymbol(value)}</option>)}</select><span aria-label={chartSource.startsWith('COINBASE') ? `Coinbase · ${status}` : `${chartSource} · ${status}`} title={`${chartSource} · ${status}`} className={`h-1.5 w-1.5 shrink-0 rounded-full ${liveClass[status]}`} /></label>
        <span role="separator" aria-orientation="vertical" className="mx-1 h-5 w-px shrink-0 bg-slate-700" />
        <label className="flex h-8 items-center"><span className="sr-only">Timeframe</span><select aria-label="Timeframe" value={timeframe} onChange={event => setTimeframe(event.target.value as Timeframe)} className="h-8 rounded-md border border-slate-800 bg-slate-950 px-2 text-xs font-semibold text-slate-200 outline-none focus-visible:ring-2 focus-visible:ring-slate-300">{TIMEFRAMES.map(value => <option key={value} value={value}>{timeframeLabel(value)}</option>)}</select></label>
