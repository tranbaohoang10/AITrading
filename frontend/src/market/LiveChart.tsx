...
export function LiveChart({ workspaceNavigation, provider = marketDataProvider }: { workspaceNavigation?: ReactNode; provider?: MarketDataProvider } = {}) {
  const auth = useAuth(), openReplay = useOpenReplay()
  ...
  return <section aria-label="Chart" data-testid="chart-view" className="relative flex h-full min-h-0 flex-col overflow-hidden">
    <header data-testid="chart-toolbar" className="flex min-h-10 shrink-0 flex-wrap items-center gap-1 sm:flex-nowrap border-b border-slate-800 bg-slate-925 px-2 py-1">
      ...
      {auth && openReplay && <ReplayLauncher account={auth.user.id} provider={activeInstrument.provider} symbol={activeInstrument.providerSymbol ?? symbol} timeframe={timeframe} onLaunch={openReplay} />}
      <div data-testid="chart-right-cluster" className="ml-auto flex w-full shrink-0 items-center justify-end gap-1 sm:w-auto"><span role="separator" aria-orientation="vertical" className="mx-1 h-5 w-px bg-slate-700" />{workspaceNavigation}<button type="button" aria-label="Chart settings" title="Chart settings" onClick={() => setSettingsOpen(true)} className={iconButton}><Icon name="settings" className="h-4 w-4" /></button><ChartClock exchangeTimezone={settings.exchangeTimezone} timezone={settings.timezone} onChange={value => updateSettings({ ...settings, timezone: value })} /><ChartUtilities hasChart={page.items.length > 0 || unitTestDefaultProvider} onRefresh={() => setAttempt(value => value + 1)} refreshing={loading} onSendAssistant={sendFullChart} /></div>
    </header>
    <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
      <ChartToolsRail positionSetup={<AnchoredPopover label="Open Position Setup" title="Position Setup" triggerContent={<Icon name="longPosition" className="h-4 w-4" />}>{close => <div className="grid gap-3 text-xs"><strong>Position Setup · Replay simulation</strong><p>Start or resume a Replay session to use its current balance and revealed price. Quantity is used while lot metadata is unavailable.</p>{openReplay && <button className="rounded border border-slate-600 p-2" onClick={() => { openReplay(); close() }}>Start / resume Replay for Position Setup</button>}</div>}</AnchoredPopover>} selected={tool} onSelect={setTool} canDelete={!!selectedDrawing} hasDrawings={drawings.length > 0} onDelete={removeSelected} onClear={() => { setDrawings([]); setSelectedDrawing(null) }} magnet={magnet} onMagnetChange={setMagnet} stayInMode={stayInMode} onToggleStay={() => setStayInMode(value => !value)} allDrawingsLocked={locked} onToggleLockAll={() => setDrawings(items => items.map(item => ({ ...item, locked: !locked })))} allDrawingsHidden={hidden} onToggleHideAll={() => setDrawings(items => items.map(item => ({ ...item, visible: hidden })))} />
