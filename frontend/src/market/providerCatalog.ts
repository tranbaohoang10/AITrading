...
const supportedCatalogProviders = new Set(['COINBASE', 'BINANCE', 'FRANKFURTER', 'ALPACA', 'DUKASCOPY', 'OANDA', 'CTRADER', 'CAPITAL'])
...
export function catalogAccess(fetcher: typeof fetch, cacheScope?: string) {
  ...
  function localCandles(raw: unknown, request: Parameters<MarketDataProvider['getHistoricalCandles']>[0]) {
  ...
  return {
    catalogProviders: async (signal?: AbortSignal): Promise<CatalogProvider[]> => {
      ...
      return raw.items.filter(p => supportedCatalogProviders.has(p.providerId) && p.displayAllowed && ['ACCEPTED', 'CONDITIONAL'].includes(p.licenseStatus))
    },
    searchPage: async (request: CatalogRequest): Promise<CatalogPage> => {
      if (!supportedCatalogProviders.has(request.provider) || request.query.length > 64) throw new Error('Invalid catalog query')
    ...
    binanceHistory: async (request: Parameters<MarketDataProvider['getHistoricalCandles']>[0]) => {
      ...
      return localCandles(raw, request)
    },
    capitalHistory: async (request: Parameters<MarketDataProvider['getHistoricalCandles']>[0]) => {
      const raw = await localHistory(request, request.symbol)
      const now = Date.now()
      return localCandles(raw, request).filter(candle => candle.closeTime < now)
    },
    providerHistory: async (provider: string, request: Parameters<MarketDataProvider['getHistoricalCandles']>[0]) => {
      if (!['ALPACA', 'OANDA', 'CTRADER', 'DUKASCOPY', 'CAPITAL'].includes(provider)) throw new Error('Unsupported history provider')
      const step = timeframeMilliseconds(request.interval), to = request.before ?? Date.now(), from = to - Math.min(1000, Math.max(1, request.limit)) * step
