...
describe('PB-006 private market UI (API contract mocks)', () => {
  it('renders persisted prices and navigates candle windows without fake indicators', async () => {
    ...
    expect(screen.getByLabelText('Selected candle values')).toHaveTextContent('100.12345678')
    expect(screen.queryByText(/EMA 50/)).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Older candles' }))
    await waitFor(() => expect(api.candles).toHaveBeenLastCalledWith(first, 100, 160))
    await screen.findByRole('img', { name: /TEST_USD imported/ })
    fireEvent.change(screen.getByLabelText('Candle window'), { target: { value: '50' } })
    await waitFor(() => expect(api.candles).toHaveBeenLastCalledWith(first, 50, undefined))
    await screen.findByRole('img', { name: /50 candles/ })
    fireEvent.click(screen.getByRole('button', { name: 'Previous candle' }))
    expect(screen.getByLabelText('Candle index')).toHaveValue('48')
    fireEvent.click(screen.getByRole('button', { name: 'Next candle' }))
    expect(screen.getByLabelText('Candle index')).toHaveValue('49')
    fireEvent.keyDown(screen.getByLabelText('Candle index'), { key: 'Home' })
    expect(screen.getByLabelText('Candle index')).toHaveValue('0')
    fireEvent.keyDown(screen.getByLabelText('Candle index'), { key: 'End' })
    expect(screen.getByLabelText('Candle index')).toHaveValue('49')
    fireEvent.keyDown(screen.getByLabelText('Candle index'), { key: 'ArrowLeft' })
    expect(screen.getByLabelText('Candle index')).toHaveValue('48')
  })
