...
it('seeds live OHLC and ignores cancelled interval history and callbacks during rapid switching', async () => {
  ...
  expect(vi.mocked(provider.subscribeCandles).mock.calls[0][0].seed).toEqual(expect.objectContaining({ interval: '1m', close: '101' }))
  ...
  expect(subscriptions.length).toBeGreaterThanOrEqual(2); expect(subscriptions.at(-1)?.interval).toBe('1m')
  expect(subscriptions.slice(0, -1).every(subscription => subscription.stop.mock.calls.length === 1)).toBe(true)
