...
it('seeds live OHLC and ignores cancelled interval history and callbacks during rapid switching', async () => {
  ...
  expect(screen.getByTestId('realtime-status')).toHaveTextContent('RealtimeWaiting for first tick')
  expect(screen.getByTestId('realtime-status')).toHaveTextContent('First tick — · Last tick — · Candle —')
  ...
  expect(screen.getByTestId('realtime-status')).toHaveTextContent(/Last(?: tick)?/)
  ...
  expect(screen.getByTestId('realtime-status')).toHaveTextContent('Candle')
  expect(screen.getByTestId('realtime-status')).toHaveTextContent('(partial)')
