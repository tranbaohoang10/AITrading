...
describe('Market Intelligence workspace', () => {
  ...
  it('switches to calendar with bounded date and timezone controls', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Display timezone'), { target: { value: 'UTC+07:00' } })
    fireEvent.click(screen.getByRole('button', { name: 'Load calendar' }))
    await screen.findByRole('alert')
    expect(api.getCalendar).toHaveBeenCalledWith(user.id, expect.any(String), expect.any(String), 'UTC+07:00')
  })
})
