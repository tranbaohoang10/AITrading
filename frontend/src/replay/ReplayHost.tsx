...
const ReplayContext = createContext<((draft?: ReplayLaunch) => void) | null>(null)
...
function OwnerReplay({ account, children }: { account: string; children: ReactNode }) {
  const [launch, setLaunch] = useState<ReplayLaunch | undefined>()
  const [open, setOpen] = useState(false)
  const [closeBlocked, setCloseBlocked] = useState(false)
  const close = () => { if (!closeBlocked) setOpen(false) }
  return <ReplayContext.Provider value={draft => { setLaunch(draft); setOpen(true) }}>{children}<Modal open={open} label="Historical Replay Trading" onClose={close}><div className="h-full w-full"><ReplayWorkspace launch={launch} account={account} onClose={close} onCloseBlocked={setCloseBlocked}/></div></Modal></ReplayContext.Provider>
}
