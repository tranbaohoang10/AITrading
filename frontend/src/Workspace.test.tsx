...
describe('TASK-003 through TASK-007 workspace behavior', () => {
  ...
  it('shows all local Chart toolbar controls', () => {
    ...
    expect(within(toolbar).getByLabelText('Indicators')).toBeInTheDocument()
    expect(within(toolbar).getByLabelText('Chart type')).toBeInTheDocument()
    expect(within(toolbar).getByLabelText('Chart layout')).toBeInTheDocument()
    expect(within(toolbar).getByRole('button', { name: 'Refresh datasets' })).toBeInTheDocument()
