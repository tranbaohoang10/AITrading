...
function OwnedNotifications({ accountId, clear, onOpenJob, locked }: { accountId: string; clear: () => void; onOpenJob: (id: string) => void; locked: boolean }) {
  ...
  return <section aria-label="Backtest notifications" aria-busy={busy} className="mb-4 rounded-md border border-slate-800 bg-slate-900 p-3 text-sm">
    <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><h3 className="font-semibold">Notifications</h3>{page && <><span className="sr-only">Unread: {page.unreadCount}</span><span className="status-chip">{page.unreadCount} unread</span></>}</div><button className={buttonClass} type="button" disabled={busy || locked} onClick={() => void run()}>{page ? 'Refresh notifications' : 'Check notifications'}</button></div>
    ...
    {page?.nextCursor && <button type="button" className={`${buttonClass} mt-3`} disabled={busy || locked} onClick={() => void run(page.nextCursor!)}>Older notifications</button>}
    <details className="help-details mt-2"><summary>Notification details</summary><p>Completion, failure and cancellation inbox. Check or refresh to retrieve updates; no realtime or external messages. Metadata is retained for 30 days. Jobs completed before notifications were enabled are not backfilled. Deleted jobs may no longer open.</p></details>
  </section>
}
