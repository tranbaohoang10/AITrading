...
it('preserves drafts through workspace aliases and mobile navigation', async () => {
  ...
  fireEvent.click(screen.getByRole('tab', { name: 'Chart' })); fireEvent.click(screen.getByRole('button', { name: 'Workspace' }))
  fireEvent.click(within(screen.getByRole('complementary', { name: 'Expanded navigation' })).getByRole('button', { name: /Quant Assistant/ }))
  expect(screen.queryByLabelText('Strategy JSON')).not.toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: 'Workspace' }))
  fireEvent.click(within(screen.getByRole('complementary', { name: 'Expanded navigation' })).getByRole('button', { name: /Strategies/ })); expect(screen.getByLabelText('Strategy JSON')).toHaveValue('persist across navigation')
