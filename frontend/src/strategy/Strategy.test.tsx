...
it('preserves drafts through workspace aliases and mobile navigation', async () => {
  ...
  fireEvent.click(screen.getByRole('tab', { name: 'Chart' })); fireEvent.click(screen.getByRole('button', { name: 'Open Quant navigation' }))
  fireEvent.click(within(screen.getByRole('complementary', { name: 'Quant navigation' })).getByRole('button', { name: 'Assistant' }))
  ...
  fireEvent.click(within(screen.getByRole('complementary', { name: 'Quant navigation' })).getByRole('button', { name: /Strategies/ })); expect(screen.getByLabelText('Strategy JSON')).toHaveValue('persist across navigation')
  act(() => { window.innerWidth = 390; window.dispatchEvent(new Event('resize')) })
  fireEvent.click(screen.getByRole('button', { name: 'Open navigation' }))
  const nav = screen.getByRole('dialog', { name: 'Quant navigation' }); fireEvent.click(within(nav).getByRole('button', { name: 'Strategies' }))
