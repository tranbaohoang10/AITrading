...
export function JournalWorkspace() {
  ...
  const openEntry = (id: string) => { journal.select(id); setEditor(true) }
        ...
        <form aria-label="Manual journal entry" onSubmit={submit} className="space-y-4">
          ...
          <fieldset disabled={locked} className="space-y-4 disabled:opacity-60">
            <div className="grid grid-cols-2 gap-3">{field('symbol', 'Symbol', true, 'TEST_USD')}
              <label className="grid gap-1.5 text-xs text-slate-400">Timeframe<select className={inputClass} aria-label="Journal timeframe" value={draft.timeframe} onChange={event => journal.edit('timeframe', event.target.value)}>{timeframes.map(tf => <option key={tf} value={tf}>{tf === '1d' ? '1D' : tf}</option>)}</select></label>
