...
export function JournalCalendar({ report, mode, onDay }: { report: api.Summary; mode: Period; onDay: (date: string) => void }) {
  ...
  return <div aria-label="Daily realized P&L" className="mt-3">
    ...
    {Array.from({ length: Math.ceil(cells.length / 7) }, (_, row) => <div key={row}>
      <div className={`grid ${mode === 'day' ? 'grid-cols-1' : 'grid-cols-7'} gap-px bg-slate-800`}>{cells.slice(row * 7, row * 7 + 7).map((day, index) => day ? <button key={day.date} aria-label={`Open day ${day.date}`} onClick={() => onDay(day.date)} className="min-w-0 bg-slate-950 px-1 py-3 text-left focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white">
...
export function JournalPagedTrades({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    let active = true
    api.numberedPage({ from, to, zone, currency }, page, size, auth?.user.id).then(value => { if (active) { setResult(value); setPage(value.page); setError('') } }).catch(() => { if (active) setError('Cannot load trade page. Retry.') })
    return () => { active = false }
  }, [from, to, zone, currency, page, size, auth?.user.id, journal.report, retry])
  return <div>
...
export function JournalDayDrawer({ date, onClose, onSelect }: { date: string | null; onClose: () => void; onSelect: (id: string) => void }) {
    ...
    const run = async () => {
      ...
      try {
        for (let page = 0; page < 25; page++) {
          ...
          if (!result.nextCursor) { setRows([...found.values()]); return }
  ...
  return <Modal open={!!date} label="Journal day" onClose={onClose}><div className="m-auto w-full max-w-3xl overflow-y-auto bg-slate-950 p-4 text-slate-100"><button onClick={onClose}>Close day</button><h2>{date} · {zone} · {currency}</h2>{error ? <p role="alert">{error}<button onClick={() => { setError(''); setRows(null); setAttempt(value => value + 1) }}>Retry day</button></p> : rows === null ? <p role="status">Loading complete day…</p> : <><p>Net P&L {signed(sumDecimals(rows.flatMap(row => row.netPnl === null ? [] : [row.netPnl])))} {currency} · {rows.length} trades</p>{rows.length === 0 && <p>No trades this day.</p>}{rows.map(row => <button className="block w-full border-b border-slate-800 py-3 text-left" key={row.id} onClick={() => onSelect(row.id)}>{row.data.symbol} · {row.data.side} · {row.data.state} · {row.netPnl ?? 'Not realized'}</button>)}</>}</div></Modal>
}
