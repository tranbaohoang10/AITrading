...
function SavedMql5({ source, accountId, dirty }: { source: Revision; accountId: string; dirty: boolean }) {
  ...
  return <section aria-label="MQL5 research export" className="flex h-full min-h-0 flex-col overflow-auto text-sm">
    <header className="space-y-3 border-b border-slate-800 p-3 sm:p-4">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="eyebrow">Code export</p><h2 className="text-base font-semibold text-slate-100">MQL5</h2><p className="mt-1 break-words text-xs text-slate-500">{source.title} · r{source.revision} · {source.symbol} / {source.timeframe}</p></div><span className="status-chip status-chip--warning">Research only</span></div>
      ...
      <details className="help-details"><summary>Export limits & risk</summary><p className="text-amber-200">Experimental MQL5 CSV research script with a closed-bar simulator. Not native Strategy Tester or live orders. Official compiler/runtime validation is pending.</p><p>Requires a UTF-8 OHLCV CSV in the local MetaTrader file sandbox, explicit symbol/timeframe confirmation and up to 5000 contiguous closed UTC bars. Double rounding can change near-threshold signals. Historical results do not guarantee profit.</p><p className="break-all font-mono">DSL SHA256: {source.hash}</p></details>
    ...
    {artifact && <pre tabIndex={0} aria-label="Generated MQL5 source" className="min-h-48 flex-1 overflow-auto bg-[#090c0b] p-4 font-mono text-xs leading-6 text-slate-300"><code>{artifact.code}</code></pre>}
  </section>
}
