...
import { Mql5Workspace } from './Mql5Workspace'
import { exportMql5 } from './api'
import { account, source, artifact } from './fixtures'
import type { Revision } from '../strategy/api'

vi.mock('./api', () => ({ exportMql5: vi.fn() }))
vi.mock('../auth/api', async original => ({ ...await original<typeof import('../auth/api')>(), currentUser: vi.fn() }))
const clear = vi.fn()
const user = { id: account, email: 'mql5@example.test', displayName: 'Synthetic' }
function view(selected: Revision | null = source, dirty = false, id = account) {
  return <AuthContext.Provider value={{ user: { ...user, id }, clear, update: vi.fn() }}><StrategyContext.Provider value={{ selected, dirty } as StrategyContextValue}><Mql5Workspace /></StrategyContext.Provider></AuthContext.Provider>
}
beforeEach(() => { vi.resetAllMocks(); vi.mocked(currentUser).mockResolvedValue(user); vi.mocked(exportMql5).mockResolvedValue(artifact) })
...
it('shows real saved provenance, inert code and explicit dirty/runtime warnings without altering draft', async () => {
  ...
  expect(await screen.findByLabelText('Generated MQL5 source')).toHaveTextContent('<script>alert(1)</script>')
  ...
  expect(exportMql5).toHaveBeenCalledWith(source, account, false)
...
it('does not request or substitute mock code for empty selection and DRAFT', () => {
  ...
  expect(exportMql5).not.toHaveBeenCalled()
})
it('generates a missing artifact and retries only the same saved version after uncertain failure', async () => {
  vi.mocked(exportMql5).mockRejectedValueOnce(new ApiError(404)).mockRejectedValueOnce(new Error('Connection interrupted')).mockResolvedValueOnce(artifact)
  render(view())
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Generate saved revision' }))
  expect(await screen.findByLabelText('Generated MQL5 source')).toBeInTheDocument()
  expect(vi.mocked(exportMql5).mock.calls).toEqual([[source, account, false], [source, account, true], [source, account, true]])
})
it('discards a late response on revision change and never clears the new session from an old failure', async () => {
  let reject!: (error: unknown) => void
  vi.mocked(exportMql5).mockReturnValueOnce(new Promise((_, fail) => { reject = fail }))
  const page = render(view()), changed = { ...source, revision: 3, hash: 'c'.repeat(64) }
  vi.mocked(exportMql5).mockResolvedValueOnce({ ...artifact, revision: 3, dslHash: changed.hash, code: 'New revision only' })
  page.rerender(view(changed))
  expect(await screen.findByLabelText('Generated MQL5 source')).toHaveTextContent('New revision only')
  await act(async () => reject(new ApiError(401)))
  expect(clear).not.toHaveBeenCalled();expect(screen.getByLabelText('Generated MQL5 source')).toHaveTextContent('New revision only')
})
it('clears stale-account responses only for the still mounted workspace and hides prior account source', async () => {
  const page = render(view());await screen.findByLabelText('Generated MQL5 source')
  let resolve!: (v: typeof artifact) => void
  vi.mocked(exportMql5).mockReturnValueOnce(new Promise(done => { resolve = done }))
  const other = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb'
  page.rerender(view(source, false, other))
  expect(screen.queryByLabelText('Generated MQL5 source')).not.toBeInTheDocument()
...
it('handles clipboard rejection without losing source and downloads inert text with bounded ASCII filename', async () => {
  ...
  const click = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(function (this: HTMLAnchorElement) { expect(this.download).toMatch(/^strategy-[a-f0-9-]+-r2-[a-f0-9]{12}\.mq5$/) })
  render(view());await screen.findByLabelText('Generated MQL5 source')
  fireEvent.click(screen.getByRole('button', { name: 'Copy MQL5 source' }))
  expect(await screen.findByRole('alert')).toHaveTextContent('Clipboard unavailable')
  expect(screen.getByLabelText('Generated MQL5 source')).toBeInTheDocument();expect(copy).toHaveBeenCalledWith(artifact.code)
  fireEvent.click(screen.getByRole('button', { name: 'Download .mq5' }))
