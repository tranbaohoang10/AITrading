...
describe('PB-004 client trust boundary', () => {
  it('rejects a mismatched conversation identity rather than mixing context', async () => {
    ...
    await expect(getMessages(id, undefined, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).rejects.toThrow('Invalid conversation response')
  })
  it('rejects malformed response shapes and timestamps', async () => {
    ...
    await expect(listConversations(undefined, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).rejects.toThrow('Invalid conversation response')
  })
  it('sends only requestId/content with CSRF, same-origin cookie, and no automatic unsafe retry', async () => {
    ...
    await expect(saveMessage(id, id, 'Hello', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).resolves.toMatchObject({ sequence: 1, role: 'user' })
  ...
  it('rejects unexpected server roles and preserves an uncertain create failure for caller retry', async () => {
    ...
    await expect(saveMessage(id, id, 'Hello', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).rejects.toThrow('Invalid conversation response')
    ...
    await expect(createConversation(id, 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).rejects.toThrow('Cannot reach')
