...
export function PersistentChat() {
  ...
  const locked = chat.busy || chat.uncertain
  return <section aria-label="AI Chat" data-testid="ai-chat" className="flex h-full min-h-0 flex-col bg-slate-900 text-slate-200">
    <header className="shrink-0 border-b border-slate-800 px-4 py-3">
      ...
      <p className="mt-1 text-xs text-slate-400">Private saved messages · AI replies not connected yet</p>
      <div className="mt-3 flex gap-2"><button className={control} disabled={chat.busy || (chat.uncertain && chat.pendingAction !== 'create')} onClick={() => void chat.create()}>{chat.pendingAction === 'create' ? 'Retry New Chat' : 'New Chat'}</button><button className={control} disabled={locked || chat.listLoading} onClick={() => void chat.loadList()}>Refresh list</button></div>
    ...
    {chat.selected ? <>
      ...
      <form className="shrink-0 border-t border-slate-800 p-3" onSubmit={event => { event.preventDefault(); void chat.save() }}>
        ...
        <button className={`${control} w-full bg-slate-200 font-semibold !text-slate-950 hover:bg-white`} disabled={chat.busy || chat.pendingAction === 'create' || !chat.draft.trim() || chat.messagesLoading} type="submit">{chat.busy ? 'Saving…' : chat.pendingAction === 'save' ? 'Retry save' : 'Save message'}</button>
      </form>
