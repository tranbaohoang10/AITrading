...
export function Values({ values, label }: { values: Detail; label: string }) {
  return <dl aria-label={label} className="grid grid-cols-1 gap-x-5 gap-y-3 text-xs sm:grid-cols-2 xl:grid-cols-3">{Object.entries(values).map(([key, value]) => <div key={key} className="min-w-0"><dt className="text-slate-500">{key.replace(/([A-Z])/g, ' $1')}</dt><dd className="break-all font-mono text-slate-200">{value === null ? 'Undefined' : String(value)}</dd></div>)}</dl>
}
export function EquityChart({ bars }: { bars: Bar[] }) {
  ...
  function curve(field: 'equity' | 'drawdownPct', title: string, color: string) {
    ...
    return <div><h4 className="mb-2 text-xs text-slate-400">{title} · bar close</h4><svg viewBox="0 0 900 155" className="h-36 w-full border border-slate-800 bg-slate-950" role="img" aria-label={title}>
  ...
  return <section aria-label="Equity and drawdown" className="space-y-4">{curve('equity', 'Equity', '#38bdf8')}{curve('drawdownPct', 'Drawdown %', '#fb7185')}
    ...
    <p className="text-xs text-slate-500">Geometry uses display precision; inspected values retain exact engine decimal strings. Drawdown can exceed 100% under leverage.</p>
  </section>
}
export function TradeList({ result }: { result: Result }) {
  ...
  return <section aria-label="Actual backtest trades" className="space-y-4">
    ...
    {result.trades.slice(from, from + 20).map((trade, i) => <details key={from + i} className="border border-slate-800 bg-slate-900/30 p-3">
...
export function ResultView({ tradesOnly = false }: { tradesOnly?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <p className="text-xs text-amber-200">{job.sourceKind} · Unverified research data. Historical results do not guarantee future profit.</p>
    ...
    <details className="border-t border-slate-800 pt-4"><summary className="cursor-pointer text-sm">Run provenance, execution policy and exact DSL</summary><div className="mt-4 space-y-5">
