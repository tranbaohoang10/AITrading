...
export function BacktestWorkspace({ tradesOnly = false }: { tradesOnly?: boolean }) {
  ...
  return <section aria-label={tradesOnly ? 'Backtest Trades' : 'Backtest Results'} className="h-full overflow-y-auto p-3 text-slate-200 sm:p-4">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">Research</p><h2 className="text-lg font-semibold">{tradesOnly ? 'Trades' : 'Backtesting'}</h2></div><div className="flex items-center gap-2"><span className={`status-chip ${context.configured ? 'status-chip--success' : ''}`}>{context.configured ? 'Engine ready' : 'Engine status'}</span><button className={buttonClass} disabled={locked} onClick={() => void context.load()}>Refresh jobs and inputs</button></div></div>
    ...
    {!tradesOnly && <details open={!context.result} className="mb-4 rounded-md border border-slate-800 bg-slate-900 p-3"><summary className="cursor-pointer text-sm font-semibold">New backtest</summary>
      <p className="my-3 text-xs text-slate-500">Select a saved VALIDATED revision and matching dataset.</p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3"><button className={`${buttonClass} primary-button`} disabled={locked || !matching || context.configured !== true} onClick={() => { setConfirm(null); void context.start() }}>Start saved backtest</button><p className="text-xs text-slate-500">{context.configured === null ? 'Check engine availability.' : !context.configured ? 'Python worker is not configured on this server.' : !matching ? 'Choose a matching gap-free dataset.' : 'Ready · no live orders'}</p></div>
    </details>}
