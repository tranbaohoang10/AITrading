...
export const generatorVersion = 'pine-research-1.0.0'
...
const invalid = () => new Error('Invalid Pine export response. Reload before continuing.')
...
export async function parseArtifact(value: unknown, source: Revision): Promise<Artifact> {
  ...
  if (bytes.length > 131072 || !code.startsWith('//@version=6\n') || !/^[0-9a-f]{64}$/.test(codeHash) || !createdAt.endsWith('Z') || !Number.isFinite(Date.parse(createdAt))) throw invalid()
  if (!globalThis.crypto?.subtle) throw new Error('Pine integrity verification requires HTTPS or localhost.')
...
const failures: Record<string, string> = {
  ...
  TARGET_RESOURCE_LIMIT: 'Pine target limit: 16 indicators and 4500 warm-up bars.',
  TARGET_PERIOD_LIMIT: 'Pine target limit: indicator period must not exceed 200.',
  TARGET_LAG_LIMIT: 'Pine target limit: operand lag must not exceed 200.',
  TARGET_OUTPUT_LIMIT: 'Generated Pine exceeds the 128 KiB target limit.',
}
export async function exportPine(source: Revision, accountId: string, create = false): Promise<Artifact> {
  ...
  try {
    const response = await privateRequest(accountId, `/strategies/${source.strategyId}/versions/${source.revision}/pine`,
    ...
    if (error instanceof ApiError && error.status === 422 && error.response) {
      ...
      throw new Error(typeof v.code === 'string' && Object.hasOwn(failures, v.code) ? failures[v.code] : 'This DSL is not supported by the Pine target.')
    }
    throw error
  }
}
