...
export function ChartClock({ timezone, onChange, exchangeTimezone = 'UTC' }: { exchangeTimezone?: 'UTC' | 'America/New_York'; timezone: ChartSettings['timezone']; onChange?: (value: ChartSettings['timezone']) => void }) {
  const [now, setNow] = useState(() => new Date())
  useEffect(() => { const timer = window.setInterval(() => setNow(new Date()), 1000); return () => window.clearInterval(timer) }, [])
  return <AnchoredPopover label="Chart clock" title="Clock / timezone" triggerContent={<Icon name="clock" className="h-3.5 w-3.5" />}>
    {close => <><p className="font-mono text-sm text-slate-100">{formatChartDate(now, timezone === 'EXCHANGE' ? exchangeTimezone : timezone, { dateStyle: 'medium', timeStyle: 'medium', hour12: false })}</p><p className="mt-1 text-[10px] text-slate-400">{timezone === 'EXCHANGE' ? `Exchange timezone · ${exchangeTimezone}` : timezone === 'LOCAL' ? `Browser local time · ${Intl.DateTimeFormat().resolvedOptions().timeZone}` : `${timezone} · ${timezoneShort(timezone)}`}</p>{onChange && <select aria-label="Clock timezone" value={timezone} onChange={event => { onChange(event.target.value as ChartSettings['timezone']); close() }} className={`${field} mt-2 w-full`}>{timezones.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select>}</>}
  </AnchoredPopover>
}
