import { hasApprovedSymbolIcon, SymbolIcon } from './SymbolIcon'
...
import { chartTimezoneOptionsAt, formatChartDate, timezoneShort } from '../market/chartTimezone'
import { useChartClock } from '../market/useChartClock'
...
export function SymbolSearchModal({ open, query, instruments, onQueryChange, onSelect, onClose }: { open: boolean; query: string; instruments: Instrument[]; onQueryChange: (value: string) => void; onSelect: (instrument: Instrument) => void; onClose: () => void }) {
  ...
  const availableCategories = ['ALL', 'STOCK', 'ETF', 'CRYPTO', 'FUTURES', 'FOREX', 'COMMODITY'] as const
  useEffect(() => { if (category !== 'ALL' && !availableCategories.includes(category)) setCategory('ALL') }, [category])
  const filtered = useMemo(() => instruments.filter(item => item.modes.includes('REALTIME') && hasApprovedSymbolIcon(item) && (category === 'ALL' || item.assetClass === category) && `${item.symbol} ${item.name} ${item.base ?? ''} ${item.quote ?? ''}`.toLowerCase().includes(query.trim().toLowerCase())), [category, instruments, query])
  if (!open) return null
  return <div role="dialog" aria-modal="true" aria-label="Symbol Search" className={symbolPanel}>
    <div className="flex items-center justify-between border-b border-slate-800 pb-3"><div><h2 className="text-lg font-semibold text-slate-100">Symbol Search</h2><p className="mt-0.5 text-[10px] text-slate-500">Choose a live-enabled market.</p></div><button type="button" aria-label="Close Symbol Search" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-md text-slate-500 hover:bg-slate-800 hover:text-white"><Icon name="close" className="h-4 w-4" /></button></div>
    ...
    <div role="tablist" aria-label="Symbol categories" className="mt-3 flex gap-2 overflow-x-auto border-b border-slate-800 pb-2">{availableCategories.map(value => { const label = value === 'ALL' ? 'All' : value === 'ETF' ? 'ETFs' : value === 'CRYPTO' ? 'Crypto' : value === 'STOCK' ? 'Stocks' : value === 'FOREX' ? 'Forex' : value === 'COMMODITY' ? 'Commodities' : 'Futures'; return <button key={value} type="button" role="tab" aria-selected={category === value} onClick={() => setCategory(value)} className={`shrink-0 rounded-md px-3 py-1.5 text-xs font-semibold ${category === value ? 'bg-slate-100 text-slate-950' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-200'}`}>{label}</button> })}</div>
    <div className="mt-2 max-h-[min(28rem,calc(100vh-16rem))] overflow-y-auto">{filtered.length ? filtered.map(item => <button key={`${item.provider}:${item.symbol}`} type="button" onClick={() => onSelect(item)} className="flex min-h-14 w-full items-center gap-3 rounded-lg px-3 text-left hover:bg-slate-800"><SymbolIcon instrument={item} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-100">{item.displaySymbol ?? displayMarketSymbol(item.symbol)}</span><span className="block truncate text-xs text-slate-500">{item.name}</span></span></button>) : <p className="px-2 py-8 text-center text-xs text-slate-500">No live symbol matches this search.</p>}</div>
  </div>
}
...
const timezones = chartTimezoneOptionsAt
