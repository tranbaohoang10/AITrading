...
export function ChartToolsRail() {
  ...
  return <aside aria-label="Chart tools" className="chart-tools flex h-10 shrink-0 items-center gap-0.5 overflow-x-auto border-b border-slate-800 bg-slate-925 px-1 sm:h-auto sm:w-10 sm:flex-col sm:overflow-visible sm:border-b-0 sm:border-r sm:px-0 sm:py-1.5">
    {tools.map(([name, label]) => <button key={name} type="button" aria-label={label} aria-pressed={selected === name} title={label} data-tooltip={label} onClick={() => setSelected(name)} className={`${toolButton} ${selected === name ? 'bg-slate-800 text-slate-100' : ''}`}><Icon name={name} className="h-4 w-4" /></button>)}
    <span className="mx-1 h-5 w-px shrink-0 bg-slate-800 sm:my-1 sm:h-px sm:w-5" aria-hidden="true" />
    <button type="button" disabled aria-label="More chart tools" title="More chart tools require a connected chart engine" data-tooltip="More tools" className={toolButton}><Icon name="more" className="h-4 w-4" /></button>
  </aside>
}
