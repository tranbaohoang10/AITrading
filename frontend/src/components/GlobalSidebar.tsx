...
export function GlobalSidebar({ onNavigate, compact = false, activeAction = 'workspace' }: { onNavigate: (action: string) => void; compact?: boolean; activeAction?: string }) {
  ...
  return <>
    <aside aria-label="Global navigation" data-testid={compact ? 'compact-sidebar' : 'global-sidebar'} className="relative z-50 flex h-screen w-[52px] shrink-0 flex-col items-center border-r border-slate-800 bg-slate-950 py-2">
      <button type="button" className="quant-mark mb-4" title="Open Quant navigation" aria-label="Workspace" aria-expanded={expanded} aria-current={activeAction === 'workspace' ? 'page' : undefined} onClick={() => { onNavigate('workspace'); setExpanded(value => !value) }}>{brand.initials}</button>
      <nav className="flex flex-1 flex-col items-center gap-1" aria-label="Primary">{navigationItems.slice(0, 5).map(item => <button key={item.action} type="button" aria-label={item.label} title={item.label} aria-current={activeAction === item.action ? 'page' : undefined} onClick={() => navigate(item.action)} className={`relative grid h-9 w-9 place-items-center rounded-md text-slate-600 transition hover:bg-slate-900 hover:text-slate-200 focus-visible:ring-2 focus-visible:ring-slate-300 ${activeAction === item.action ? 'bg-slate-800 text-white after:absolute after:-left-2 after:h-4 after:w-px after:bg-slate-300' : ''}`}><Icon name={item.icon} /></button>)}</nav>
      <button type="button" aria-label="Account" title={auth?.user.displayName ?? 'Account'} aria-current={activeAction === 'account' ? 'page' : undefined} onClick={() => navigate('account')} className={`grid h-8 w-8 place-items-center rounded-full border text-[9px] font-semibold transition ${activeAction === 'account' ? 'border-slate-400 bg-slate-700 text-white' : 'border-slate-800 bg-slate-900 text-slate-400 hover:border-slate-600'}`}>{initials}</button>
    </aside>
